					...
					foreach ( $this->choices as $key => $value ) {
						if ( is_array( $value ) ) {
							...
							foreach ( $value as $optgroupkey => $optgroupvalue ) {
								echo '<option value="' . esc_attr( $optgroupkey ) . '" ' . ( in_array( esc_attr( $optgroupkey ), $defaultValue ) ? 'selected="selected"' : '' ) . '>' . esc_html( $optgroupvalue ) . '</option>';
							}
							echo '</optgroup>';
						} else {
							echo '<option value="' . esc_attr( $key ) . '" ' . selected( esc_attr( $key ), ( in_array( $key, $defaultValue ) ? $key : false ), false )  . '>' . esc_html( $value ) . '</option>';
						}
 					}
	 				?>
				</select>
